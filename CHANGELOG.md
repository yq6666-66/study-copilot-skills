...
## v1.13 — 2026-09-14（可行性验证与提交材料）

### 新增
- `dist/run_demo.py`：评委视角 21 步全链路可行性实跑（测试/三门禁/数据校验/真实检索/FSRS 调度/周报/仪表盘/Anki/预练/离线降级/安装器），21/21 通过；`SKIP_LLM` 幂等开关防重跑刷调用。
- `docs/competition/初赛提交材料模板.md`：作品介绍、AI 技术实践说明（端云分工与协同方式，按 BONUS 规则）、运行/安装/登录三要素、规则对照自检表——提交表单可直接复制。
- 可行性实跑产生第 33/34 次真实 Qwen 留痕（预练定向变式）；材料计数同步 34 次 / 79,350 tokens。
