...
## v1.12 — 2026-09-14（提交材料对齐）

### 新增
- **06-DashScope 控制台用量证据**：千问AI平台官方「按量付费」页面两张截图（近 1 周 390 请求 / 226.9M tokens / 成功率 100%；392 条调用日志明细）入库 `docs/competition/控制台用量/`，打包脚本自动收进作品包 `AI实践验证文件夹/06-DashScope控制台用量/`（附口径说明：作品链路 32 次为控制台总量真子集，会话级用量单列不混用）。
- AI 技术实践说明新增「六、平台控制台交叉核验」节；证据总览核验方法同步。
- 体验说明按比赛要求补「运行/安装/登录方式」三要素导语（无账号、无登录、可选环境变量降级）。

## v1.11 — 2026-09-14（第十六轮：考前预练 CLI）

### 新增
- `scripts/pre_drill.py`：三引擎串联的考前预练——端侧 bge 语义召回相似历史错题（带相似度与错因摘录）→ 云端 Qwen 定向变式预练卷（`--qwen`，第 31/32 次留痕 `20260914-041149`/`20260914-041238`）→ Markdown 输出；`--no-qwen` 离线模式；`--subject/--topic/--top-k/--fake` 选项。
- 示例 `docs/competition/考前预练示例.md`（召回 r028 相似度 0.814 领衔）。
- 8 个回归测试（召回过滤/预练卷结构/Qwen 注入/互斥旗标/缺索引/路径守卫）；frontmatter 摘录瑕疵修复。全套 92 passed。
- 材料计数 32 次 / 77,765 tokens（×26/×6）。
