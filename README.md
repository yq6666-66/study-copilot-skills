...
-  **Anki 导出器**（新增）：`scripts/export_anki.py` 把错题队列导出为 Anki 可导入 CSV（含 FSRS 稳定性/难度/建议复习日字段；Front 只含回忆问题）；`--qwen-prompts` 可选由 Qwen 改写卡片措辞并留痕（示例 `docs/competition/anki导出示例.csv`）

## Qwen 使用实录

以下 **28 次调用**均为对 DashScope（`qwen-flash` 与 `qwen3.8-flash`）的真实请求，留痕（消息/回复/token/时延）存于 [`logs/qwen/`](logs/qwen/)，生成物均已入库；本节初稿亦由 Qwen 撰写（第 10 次）后经人工校对。
...
覆盖科目域：数学一 / 408（数据结构·组成原理·操作系统）/ 英语一 / 政治 / 法考 / 注册会计师；模型：`qwen-flash` ×22、`qwen3.8-flash` ×6。
