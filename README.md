...
### 会话级深度优化（qwen3.8-flash 直接驱动本项目开发）

除 API 调用外，本项目的两项深度优化由 **qwen3.8-flash 作为 ZCode 编程会话的驱动模型直接完成**（2026-09-14）：端侧检索向量缓存（59 篇文档重建 2.2s → 0.0s，增量零重算）、引擎 `--stats` 留痕聚合（22 次调用一键审计，API Key 泄漏数 0），附 8 个回归测试。完整证据与核验方式见 [docs/competition/Qwen会话优化实录.md](docs/competition/Qwen会话优化实录.md)。
