...
以下 **34 次调用**均为对 DashScope（`qwen-flash` 与 `qwen3.8-flash`）的真实请求，留痕（消息/回复/token/时延）存于 [`logs/qwen/`](logs/qwen/)，生成物均已入库；本节初稿亦由 Qwen 撰写（第 10 次）后经人工校对。
...
| 33 | 可行性实跑预练（qwen-flash） | `dist/run_demo.py` 评委视角全链路验证 | `20260914-044112` |
| 34 | 可行性实跑预练 v2（qwen-flash） | 同上（幂等修正后 21/21） | `20260914-044146` |

覆盖科目域：数学一 / 408（数据结构·组成原理·操作系统）/ 英语一 / 政治 / 法考 / 注册会计师；模型：`qwen-flash` ×28、`qwen3.8-flash` ×6。
