...
学习副驾 Study Copilot 是一个可装进 Codex、Claude Code 等 AI Agent 宿主的通用学习认知引擎：底层由 `plugins/study-copilot/skills/`（15 个 Skill）与 `plugins/study-copilot/references/`（23 份契约与 Schema）构成不依赖后台进程的 Skills 核心；中间由 `subjects/` 提供科目配置，决定科目、大纲主题与学科教练映射；上层由 `adapters/` 对接宿主运行时。运行时采用端云协同：本地通过 `scripts/local_retrieval/`（bge-small-zh ONNX，CPU）完成语义检索与索引，学习数据沉淀在用户自己的 Vault（演示库 `demo-vault/`）；需要推理增强时，才经 `scripts/qwen_engine.py` 调用云端大模型（任意 OpenAI 兼容服务，由环境变量配置）——任一端不可用都会自动降级，不阻塞学习。
...
flowchart TD
  ...
  subgraph CLOUD["云端增强（可降级）"]
    CLOUD_ENGINE["qwen_engine.py → 云端模型（OpenAI 兼容）"]
  end
...
| 路径 | 职责 |
| --- | --- |
| `plugins/study-copilot/skills/` | 15 个主责 Skill：错题闭环、规划、执行、诊断、模考、学科教练×4、真题搜索/分析、材料助手、官方核验、云端陪练、科目接入向导 |
...
| `subjects/` | 科目配置层：`kaoyan/`（6 科）、`fakao/`、`cpa/`（后两者由云端模型生成入库）、`TEMPLATE/` + 接入向导 |
...
| `scripts/qwen_engine.py` | 云端模型引擎：任意 OpenAI 兼容接口（`OPENAI_BASE_URL`/`OPENAI_MODEL` 配置）、密钥候选链与 401 自动切换、超时重试、调用全留痕且日志防 Key 泄漏、`--stats` 审计 |
| `scripts/demo_pipeline.py` | 一键端云协同演示：锚点错题→端侧召回→云端变式题→留痕总结（`--dry-cloud`/`--fake` 可离线） |
...
| `demo-vault/` | 脱敏演示记忆库（40 条错题×2 科目 + 云端生成原创练习，全部原创虚构） |
...
本地优先：`demo_pipeline.py` 展示完整链路——端侧召回（真实实测 0.03s）→ 把错因与相似簇组织为最小 prompt → `--dry-run` 隐私预览 → 真实调用云端模型产出原创变式题 → 自动留痕 `logs/qwen/`。失败降级：无 Key/超时则明示「本次未使用云端模型」，会话内继续出题，绝不伪造 `[AI生成]` 标注。
...
4. **隐私边界**——学习记录只存用户本地；云端只接收学科文本；无遥测；调用日志本地留存可自删（`PRIVACY.md`）。
5. **密钥卫生**——API 密钥（`OPENAI_API_KEY` 等）只从环境变量/Windows 用户级注册表读取（跨平台优雅降级）；引擎内置断言保证留痕日志不含 Key；semgrep 门禁（CI）阻断凭据与用户路径入库。
