...
| 云端推理（专项链路） | **阿里云 Qwen（DashScope，qwen-flash ×19 + qwen3.8-flash ×6）** | 错题陪练、科目配置生成（法考/注会入库）、原创内容生产（练习集×2/英语写作/政治卡片）、数据分析（聚类/周报/复测计划/卷种差异）、工程辅助（代码审查×3、双模型对比、接入教程、FAQ、README 实录、**ARCHITECTURE 初稿**、**CLI 错误文案评审**）、**提示词与代码优化落地**（qwen3.8-flash 优化 Skill 措辞入库；审查意见 4 条采纳改码附 3 回归测试）、**英文 README 初稿**——**共 32 次真实调用全部留痕，累计 77,765 tokens** | `logs/qwen/*.json`、[千问证据总览](千问证据总览.md)、[千问链路全景](千问链路全景.md) |
...
**口径区分**：本作品代码链路的调用为 **32 次**（`logs/qwen/` 逐一留痕：qwen-flash ×26 + qwen3.8-flash ×6，聚合器 `scripts/qwen_engine.py --stats` 可复核），是控制台总量的真子集；其余为开发期间以 Qwen 作为编程会话模型产生的用量。两类用途分开陈述——评委既能在控制台看到账号级真实用量，也能在包内把 32 条留痕与日志页逐条对上。
