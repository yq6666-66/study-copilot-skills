...
| 开发辅助 | GLM + Qwen 辅助 | 开发过程辅助编码；法考/注会配置包、错因聚类报告、FAQ 等由 Qwen 生成入库；**深度优化两项（端侧向量缓存、引擎 --stats 留痕聚合）由 qwen3.8-flash 作为 ZCode 编程会话驱动模型直接完成**（见 [Qwen会话优化实录](Qwen会话优化实录.md)，会话 ID + 模型 I/O 日志本机可验） | 提交历史、`logs/qwen/` 对应留痕 |
