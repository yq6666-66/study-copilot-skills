...
| 云端推理（专项链路） | **阿里云 Qwen（DashScope，qwen-flash ×28 + qwen3.8-flash ×6）** | 错题陪练、科目配置生成（法考/注会入库）、原创内容生产（练习集×2/英语写作/政治卡片）、数据分析（聚类/周报/复测计划/卷种差异）、工程辅助（代码审查×3、双模型对比、接入教程、FAQ、README 实录、**ARCHITECTURE 初稿**、**CLI 错误文案评审**、**英文 README 初稿**、**仪表盘摘要与预练卷×4**）、**提示词与代码优化落地**（qwen3.8-flash 优化 Skill 措辞入库；审查意见 4 条采纳改码附 3 回归测试）——**共 36 次真实调用全部留痕，累计 81,379 tokens** | `logs/qwen/*.json`、[千问证据总览](千问证据总览.md)、[千问链路全景](千问链路全景.md) |
| 端侧推理（AI PC） | **bge-small-zh-v1.5（ONNX，CPU）本地 embedding** | 错题队列与 Vault 笔记的语义向量索引与相似检索 | `.index/` 产物、检索输出截图 |
| 开发辅助 | GLM + Qwen 辅助 | 开发过程辅助编码；法考/注会配置包、错因聚类报告、FAQ 等由 Qwen 真实生成并入库；**两轮深度优化由 qwen3.8-flash 作为 ZCode 编程会话驱动模型直接完成**：向量缓存、--stats 审计、新建 CI（首跑抓出 2 个跨平台 bug 修复转绿，Actions 记录公开可查）、增量索引、检索质量端到端测试，34 tests（见 [Qwen会话优化实录](Qwen会话优化实录.md)） | 提交历史、`logs/qwen/` 留痕、GitHub Actions 运行记录 |
...
**口径区分**：本作品代码链路的调用为 **34 次**（`logs/qwen/` 逐一留痕：qwen-flash ×28 + qwen3.8-flash ×6，聚合器 `scripts/qwen_engine.py --stats` 可复核），是控制台总量的真子集；其余为开发期间以 Qwen 作为编程会话模型产生的用量。两类用途分开陈述——评委既能在控制台看到账号级真实用量，也能在包内把 34 条留痕与日志页逐条对上。
